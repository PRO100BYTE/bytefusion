...
function* handleError(e) {
  ...
  yield put(setRequestStatus(e?.message || 'Не уладось загрузить товары'));
  console.log('ERROR: ', e);
}
  ...
  switch (type) {
    ...
    case ADD_PRODUCT: {
      try {
        ...
        yield handleAction(ADMIN_PRODUCTS, 'Товар успешно добавлен', 'success');
        yield put(setLoading(false));
      } catch (e) {
        yield handleError(e);
        yield handleAction(undefined, `Не удалось добавить товар: ${e?.message}`, 'error');
      }
      break;
    }
    case EDIT_PRODUCT: {
      try {
        ...
        yield handleAction(ADMIN_PRODUCTS, 'Товар успешно отредактирован', 'success');
        yield put(setLoading(false));
      } catch (e) {
        yield handleError(e);
        yield handleAction(undefined, `Не удалось отредактировать товар: ${e.message}`, 'error');
      }
      break;
    }
    case REMOVE_PRODUCT: {
      try {
        ...
        yield put(setLoading(false));
        yield handleAction(ADMIN_PRODUCTS, 'Товар успешно удален', 'success');
      } catch (e) {
        yield handleError(e);
        yield handleAction(undefined, `Не удалось удалить товар: ${e.message}`, 'error');
      }
      break;
    }
    case SEARCH_PRODUCT: {
      try {
        ...
        if (result.products.length === 0) {
          yield handleError({ message: 'Товары не найдены.' });
          yield put(clearSearchState());
        } else {
    ...
    default: {
      throw new Error(`Непредвиденный тип действия ${type}`);
    }
  }
}

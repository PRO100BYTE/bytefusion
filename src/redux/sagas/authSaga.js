...
function* handleError(e) {
  ...
  switch (e.code) {
    case 'auth/network-request-failed':
      yield put(setAuthStatus({ ...obj, message: 'Упс... Сетевая ошибка, попробуйте чуть позже.' }));
      break;
    case 'auth/email-already-in-use':
      yield put(setAuthStatus({ ...obj, message: 'Данный адрес электронной почты уже используется. Попробуйте указать другой.' }));
      break;
    case 'auth/wrong-password':
      yield put(setAuthStatus({ ...obj, message: 'Неправильный адрес электронной почты или пароль' }));
      break;
    case 'auth/user-not-found':
      yield put(setAuthStatus({ ...obj, message: 'Неправильный адрес электронной почты или пароль' }));
      break;
    case 'auth/reset-password-error':
      yield put(setAuthStatus({ ...obj, message: 'Не удалось отправить письмо с ссылкой для сброса пароля. Проверьте правильность указанного адреса электронной почты' }));
      break;
    default:
  ...
  switch (type) {
    ...
    case RESET_PASSWORD: {
      try {
        ...
        yield put(setAuthStatus({
          success: true,
          type: 'reset',
          message: 'Письмо с ссылкой для сброса пароля было отправлено на указанный Вами адрес электронной почты'
        }));
        yield put(setAuthenticating(false));
      } catch (e) {
    ...
    case ON_AUTHSTATE_SUCCESS: {
      ...
      if (snapshot.data()) { // if user exists in database
        ...
        const user = {
          fullname: payload.displayName ? payload.displayName : 'Пользователь',
      ...
      yield put(setAuthStatus({
        ...
        type: 'auth',
        isError: false,
        message: 'Успешная авторизация! Перенаправление...'
      }));
      yield put(setAuthenticating(false));
      break;
    }
    ...
    default: {
      throw new Error('Непредвиденный тип действия.');
    }
  }
}

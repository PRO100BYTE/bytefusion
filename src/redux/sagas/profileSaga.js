  ...
  switch (type) {
    case UPDATE_EMAIL: {
      try {
        ...
        yield call(displayActionMessage, 'Адрес электронной почты успешно обновлен!', 'success');
      } catch (e) {
    ...
    case UPDATE_PROFILE: {
      try {
        ...
        yield call(displayActionMessage, 'Профиль успешно обновлен!', 'success');
      } catch (e) {
        console.log(e);
        yield put(setLoading(false));
        if (e.code === 'auth/wrong-password') {
          yield call(displayActionMessage, 'Неверный пароль, профиль не обновлен :(', 'error');
        } else {
          yield call(displayActionMessage, `Не удалось обновить профиль :( ${e.message ? e.message : ''}`, 'error');
        }
      }
      break;
    }
